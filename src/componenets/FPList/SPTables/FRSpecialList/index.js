...
const FRSpecialList = () => {
  ...
  const update_trfr_list = async () => {
    if (trashList?.length > 150) {
      setSnackbar({
        open: true,
        message:
          "لیست آشفال شما پر می‌باشد. لطفا لیست آشفال خود را مدیریت نمایید ",
        severity: "warning",
      });
    } else {
      try {
  ...
  const exportPDF = () => {
    ...
    doc.text(
      "لیست رشته‌های پیشنهادی آزاد باآزمون",
      pageWidth - margin,
      margin_now,
      {
        align: "right",
            ...
            >
              ...
              <Tooltip title="سطل آشغال">
                <Button
                  ...
                  onClick={() => navigate("/trfr/" + stu_id + "/" + field)}
                >
