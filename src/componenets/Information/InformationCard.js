...
const InformationCardContainer = styled.div`
  ...
  margin: 0px 20px;
  background: ${(props) =>
    GetBackGroundColor(props.background) || "transparent"};
...
const InformationCard = ({ iconSrc, title, color, path }) => {
  const navigate = useNavigate();
  return (
    <InformationCardContainer background={color} onClick={() => navigate(path)}>
      {iconSrc}
